...
def do(passwd):
    ...
    for i in res:
        ...
        if sha1[5:] == j[0]:
            print(
                "Oh no! Pwned. Your password has been listed password dictionaries.\nYou should change this password immediately.\n"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Check if your password is on password dictionaries."
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    parser.add_argument(
        metavar="PASSWORD", dest="passwd", help="Your password which will be checked."
    )
