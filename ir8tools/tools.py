...
def main():
    parser = argparse.ArgumentParser(
        description="A collection of scripts/tools", add_help=False
    )
    parser.add_argument(
        "-l",
        "--list",
        dest="toollist",
        action="store_true",
        help="Show the list of tools.",
    )
    parser.add_argument(
        "tool",
        nargs="*",
        metavar="TOOL",
        help="Name of the tool you want to run.",
        default=[],
    )
    ...
    parser.add_argument(
        ...
        "--help",
        dest="help",
        help="Show help for perticular tool or this message if no tool was given.",
        action="store_true",
    )
    parser.add_argument(
        "-v",
    ...
    if (
        args.help and "echoargs" == args.tool[-1]
    ):  # echoargs does not use argparse so printing help for it.
